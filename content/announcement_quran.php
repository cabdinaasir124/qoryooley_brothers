        ...
        <div class="d-flex justify-content-between align-items-center">
          <div>
            ...
            <p class="text-muted mb-1">
              Posted by ustaad <?= htmlspecialchars($row['posted_by']) ?> | 
              <?= date('d M Y', strtotime($row['created_at'])) ?>
            </p>
            <p class="mb-0"><?= nl2br(htmlspecialchars($row['body'])) ?></p>
          </div>
          <div>
            <span class="badge bg-success">Qur’an</span>
          </div>
        </div>
      </div>
    </div>
<?php
  ...
  echo '<div class="alert alert-warning">No Qur’an announcements found.</div>';
endif;
