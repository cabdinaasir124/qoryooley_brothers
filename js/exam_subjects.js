document.addEventListener('DOMContentLoaded', function () {
  ...
  const tableEl = document.getElementById('examSubjectsTable'); // table element
  ...
  function rowHtml(row) {
    const examLabel = row.exam_title ?? '';
    return `
      <tr>
        <td>${row.id}</td>
        <td>${examLabel}</td>
        ...
        <td>${row.exam_date ?? ''}</td>
        <td>${row.max_marks ?? ''}</td>
        <td><button class="btn btn-sm btn-warning" data-id="${row.id}" data-action="edit"><i class="fas fa-edit"></i>&nbsp;Edit</button></td>
        <td><button class="btn btn-sm btn-danger" data-id="${row.id}" data-action="delete"><i class="fas fa-trash"></i>&nbsp;Delete</button></td>
      </tr>
    `;
  }
    ...
    fetch('../api/exam_subjects_api.php?action=list')
      ...
      .then(data => {
        ...
        dataTable = $(tableEl).DataTable({
          ...
          responsive: true,
          columnDefs: [
            { orderable: false, targets: [8, 9] } // make edit/delete columns unsortable
          ]
        });
      });
  }
